...
from selenium_utils.config import LOG_TRACE

logger = logging.getLogger(__name__)

class ElementCheckError(Exception): pass
class InvalidByException(Exception): pass
class ElementWaiter(Protocol):
    def __call__(self, selector: str, by: str = "css selector", timeout: int | None = None) -> None: ...
...
class SeleniumSelector:
    ...
    def _is_element_stale(self) -> bool:
        if not self._web_element: return True
        try:
    ...
    def find_element(self, sb: SB, subelement: "SeleniumSelector | None" = None) -> WebElement:
    ...
    def find_elements(self, sb: SB, subelement: "SeleniumSelector | None" = None) -> List[WebElement]:
        if subelement:
    ...
    def archive(self, sb: SB, filename: str, highlight: "SeleniumSelector | None" = None) -> tuple[Path, Path] | None:
        from selenium_utils.archive import element_archive
        if self._taking_archive: return None
    ...
    def _run_element_check(self, check_func: ElementWaiter | ElementIs, timeout: int = settings.SMALL_TIMEOUT) -> None:
        try:
            check_func(*self.get(), timeout=timeout)
        except Exception as e:
            if type(e).__name__ == "Exception":
                if "clickable" in str(e): raise ElementNotInteractableException from e
                if "visible" in str(e): raise ElementNotVisibleException from e
                if "present" in str(e): raise NoSuchElementException from e
                if "enabled" in str(e): raise InvalidElementStateException from e
            raise

    def _is_element_check(self, sb: SB, descriptor: str = None, check_func: ElementWaiter | ElementIs = None, timeout: int = settings.SMALL_TIMEOUT, archive_success: bool = True, archive_failed: bool = True) -> bool:
        if not descriptor: descriptor = str(self)
        if not check_func: check_func = sb.wait_for_element_present

        match check_func:
            case sb.wait_for_element_present: check_type = "PRESENT"
            case sb.wait_for_element_visible: check_type = "VISIBLE"
            case sb.is_element_enabled: check_type = "ENABLED"
            case sb.wait_for_element_clickable: check_type = "CLICKABLE"
            case _: raise ValueError(f"Invalid check_func")

        exception_to_type = {
            NoSuchElementException: "FOUND", ElementNotVisibleException: "VISIBLE",
            ElementNotInteractableException: "CLICKABLE", InvalidElementStateException: "ENABLED",
        }

        try:
            self._run_element_check(check_func, timeout)
            if archive_success: self.archive(sb, f"{descriptor}_{check_type}")
            return True
    ...
    def is_present(self, sb: SB, descriptor: str = None, timeout: int = settings.SMALL_TIMEOUT, archive_success: bool = True, archive_failed: bool = True) -> bool:
        return self._is_element_check(sb, descriptor, sb.wait_for_element_present, timeout, archive_success, archive_failed)

    def is_visible(self, sb: SB, descriptor: str = None, timeout: int = settings.SMALL_TIMEOUT, archive_success: bool = True, archive_failed: bool = True) -> bool:
        return self._is_element_check(sb, descriptor, sb.wait_for_element_visible, timeout, archive_success, archive_failed)

    def is_clickable(self, sb: SB, descriptor: str = None, timeout: int = settings.SMALL_TIMEOUT, archive_success: bool = True, archive_failed: bool = True) -> bool:
        return self._is_element_check(sb, descriptor, sb.wait_for_element_clickable, timeout, archive_success, archive_failed)

    def is_enabled(self, sb: SB, descriptor: str = None, timeout: int = settings.SMALL_TIMEOUT, archive_success: bool = True, archive_failed: bool = True) -> bool:
        return self._is_element_check(sb, descriptor, sb.is_element_enabled, timeout, archive_success, archive_failed)

    def hide(self, sb: SB, description: str = None) -> None:
        if not description: description = str(self)
        if not self.is_visible(sb, description, archive_success=False, archive_failed=False): return
        self.find_element(sb)
        self.scroll_to(sb)
        try:
            sb.safe_execute_script("arguments[0].style.display='none';", self._web_element)
        except Exception:
            pass
...
def create_xpath_from_element(element: WebElement) -> str | None:
    try:
        ...
        attributes = {name["name"]: name["value"] for name in element.get_property("attributes")}
        xpath_parts = [f"//{tag_name}"]
        attribute_parts = [f"@{k}='{v}'" for k, v in attributes.items() if k != "class"]
        if attribute_parts: xpath_parts.append(f"[{' and '.join(attribute_parts)}]")
...
def create_selector_for_parent_with_child_id(element: WebElement, name: str) -> SeleniumSelector | None:
    try:
        children = element.find_elements(By.XPATH, ".//*[@id]")
        if not children: return None
        child_id = children[0].get_attribute("id")
        parent_tag = element.tag_name.lower()
        css = f"{parent_tag}:has(#{child_id})" if is_valid_css_id(child_id) else f'{parent_tag}:has([id="{child_id}"])'
...
def get_selenium_selector(sb: SB, element: WebElement, name: str) -> SeleniumSelector:
    ...
    if element_id: return SeleniumSelector(By.ID, element_id, name)
    temp = create_selector_for_parent_with_child_id(element, name)
    if temp and len(temp.find_elements(sb)) == 1: return temp
    element_classes = element.get_attribute("class").replace(" ", ".")
    if element_classes:
        temp = SeleniumSelector(By.CSS_SELECTOR, f"{element.tag_name}.{element_classes}", name)
        if len(temp.find_elements(sb)) == 1: return temp
    return SeleniumSelector(By.XPATH, create_xpath_from_element(element) or f"//{element.tag_name}", name)
