...
def _get_log_levels():
    ...
    file_level = _log_level_to_int(
        os.environ.get("FILE_LEVEL", _log_level_to_name(file_level)).upper()
    )
    console_level = _log_level_to_int(
        os.environ.get("CONSOLE_LEVEL", _log_level_to_name(console_level)).upper()
    )
    return file_level, console_level


def _create_file_handler(
    formatter: logging.Formatter, level: int, log_dir: Path
) -> logging.FileHandler:
...
def _create_memory_handler(
    formatter: logging.Formatter, log_dir: Path
) -> MemoryHandler:
    ...
    memory_handler = MemoryHandler(
        capacity=10000, flushLevel=logging.ERROR, target=target_handler
    )
