...
def get_most_recent_project_frame(project_root: str = None) -> tuple[str, str, int] | None:
    ...
    if last_project_frame:
        return (last_project_frame.tb_frame.f_code.co_filename, last_project_frame.tb_frame.f_code.co_name, last_project_frame.tb_lineno)
    frame = exc_traceback
    while frame.tb_next:
        frame = frame.tb_next
    return (frame.tb_frame.f_code.co_filename, frame.tb_frame.f_code.co_name, frame.tb_lineno)

def handle_exception(message: str, exc_obj: Exception, sb: SB = None, name: str = None, zoom: SeleniumSelector = None, **kwargs: Any) -> Path | None:
    increment_exit_code()
    ...
    try:
        ...
        send_webhook(
            WebhookConfig(title="EXCEPTION SOURCE", description=message or e_vars.get("MESSAGE"), image_filepath=image_filepath, additional_fields=kwargs),
            Url=current_url, EXIT_CODE=EXIT_CODE, **e_vars
        )
...
def write_exception_to_file(filepath: Path, exception: Exception = None, additional_message: str = None) -> None:
    try:
...
def _log_traceback_frames(e: Exception, e_vars: dict[str, Any]) -> None:
    try:
        ...
        while tb:
            ...
            e_vars[f"Traceback frame #{tbi:02}"] = f"File: {tb.tb_frame.f_code.co_filename}, Line: {tb.tb_lineno}\nFunction: {tb.tb_frame.f_code.co_name}\nCode: {code_line}"
            tb = tb.tb_next
    except Exception:
        pass

def _handle_selenium_details(sb: SB = None, name: str = "EXCEPTION", zoom: SeleniumSelector = None) -> tuple[str, Path]:
    current_url = name
...
def _log_frame_details(e: Exception) -> None:
    ...
    try:
        ...
        while tb:
            tbi += 1
            code_line = linecache.getline(tb.tb_frame.f_code.co_filename, tb.tb_lineno)
            logger.log(LOG_TRACE, "Frame[%d] %s in %s:%s [%s]", tbi, tb.tb_frame.f_code.co_name, tb.tb_frame.f_code.co_filename, tb.tb_lineno, code_line, extra={"Frame Vars": tb.tb_frame.f_locals})
            tb = tb.tb_next
    except Exception:
        pass
...
def _extract_exception_details(exc_obj: Exception) -> dict[str, Any]:
    ...
    try:
        ...
        if frame_info:
            ...
            e_vars["CODE"] = linecache.getline(e_vars["FILE"], int(e_vars["LINE"])).strip()
        e_vars["TYPE"] = str(sys.exc_info()[0])
        e_vars["MESSAGE"] = getattr(exc_obj, "msg", str(exc_obj))
        e_vars["TRACEBACK"] = "".join(traceback.format_exception(type(exc_obj), exc_obj, exc_obj.__traceback__))
    except Exception:
        pass
    return e_vars
