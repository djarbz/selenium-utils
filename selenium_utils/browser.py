...
def navigate_url(sb: SB, url: str, desc: str = "page", timeout: int = 30) -> None:
    try:
        ...
        if not scheme or not netloc:
            ...
            if not scheme: scheme = current_parsed.scheme or "https"
            if not netloc: netloc = current_parsed.netloc

        full_url = urlunparse((scheme, netloc, parsed_url.path, parsed_url.params, parsed_url.query, parsed_url.fragment))
    except Exception as e:
